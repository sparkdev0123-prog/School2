    ...
    (function () {
      ...
      const galleryData = {
        "republic-day": {
          title: "Republic Day",
          type: "grid",
          images: buildFolderImages("republic-day", 17)
        },
        "christmas": {
          title: "Christmas",
          type: "grid",
        ...
        "independence-day": {
          ...
          type: "grid",
          images: buildFolderImages("independence-day", 16)
        }
      };
      ...
      function openGallery(key) {
        ...
        if (!data.images || data.images.length === 0) {
          ...
          data.images.forEach(function (src, idx) {
            ...
            // Eager loading prevents broken/blank thumbnails inside the scrollable modal.
            img.loading = "eager";
            btn.appendChild(img);
...
(function () {
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    trails[0].y = mouseY;
    trails.forEach((t, i) => {
      t.el.style.opacity = String(0.42 - i * 0.07);
    });
  }, { passive: true });
  ...
  function animateCursor() {
    ...
    for (let i = 1; i < trails.length; i++) {
      ...
      current.x += (prev.x - current.x) * 0.28;
      current.y += (prev.y - current.y) * 0.28;
  ...
  document.addEventListener("mouseover", (e) => {
    if (e.target.closest("a, button, .card, .gallery-item")) {
